//$Id: visit.vtkreader.cpp 4865 2014-01-01 15:49:23Z starviewer $
...
VTKReader::VTKReader(string flnm) : vtkPolyDataAlgorithm
{
    filename = flnm;
}
...
int VTKReader::is_whitespace(char c)
{
    switch(c)
    {
        case ' ':
...
int VTKReader::RequestData(vtkInformation* vtkNotUsed(request),
                        ...
                        if(sscanf(pChar,"%d/%d/%d",&iVert,&iTCoord,&iNormal)==3) 
                        {
                            if ((iVert < 0) || (iTCoord < 0) || (iNormal < 0))
                            {
                                EXCEPTION2(InvalidFilesException, filename,
                                           "VisIt does not support relative references in OBJ files.");
                            }
