//$Id: hdf2dviewer.cpp 5249 2014-12-08 18:27:34Z starviewer $

#include <QtOpenGL>

#include <hdf2dviewer.h>

HDF2dViewer::HDF2dViewer(ColorTable *ct, NVOptions* opt)
{
    colorTable = ct;
    nvoptions = opt;
...
void HDF2dViewer::reset_texture1d(ColorTable *ct)
{
    colorTable = ct;
...
void HDF2dViewer::set_geometry(HDFGeometry *gm)
{
    geometry = gm;
...
void HDF2dViewer::setup(string vn, double *var)
{
    reset();
...
void HDF2dViewer::_initialize()
{
    int i, j, n;

    nlon = geometry->get_nlon();
    nlat = geometry->get_nlat();
    nlev = geometry->get_nlev();
    ...
    if((NULL == _x2d) || (NULL == _y2d))
    {
        lon = geometry->get_lon();
        lat = geometry->get_lat();

        _x2d = (double**)calloc(nlat, sizeof(double*));
        _y2d = (double**)calloc(nlat, sizeof(double*));

        for(j = 0; j < nlat; ++j)
        {
            _x2d[j] = (double*)calloc(nlon, sizeof(double));
            _y2d[j] = (double*)calloc(nlon, sizeof(double));
            ...
            {
		if(0.0 > lon[n + i])
                    _x2d[j][i] = lon[n + i] / 180.0 + 1.0;
		else
                    _x2d[j][i] = lon[n + i] / 180.0 - 1.0;
                _y2d[j][i] = lat[n + i] / 180.0;
            }
        }
    }
...
void HDF2dViewer::_lonlat2xyz(double lon, double lat, double radius, double f)
{
...
void HDF2dViewer::_lonlat2flat(double lon, double lat, double radius, double f)
{
...
void HDF2dViewer::_displayZsphere(int zl)
{
    int i, j;
...
void HDF2dViewer::_displayXsphere(int xl)
{
    int j, k, n;
...
void HDF2dViewer::_displayYsphere(int yl)
{
    int i, k, n;
...
void HDF2dViewer::_displayZflat(int zl)
{
    int i, im1, j, k, m, n;
  ...
  //cout << "\t_zlist[k] = " << _zlist[k] << endl;
    ...
    for(j = 0; j < nlat-1; ++j)
    {
        ...
        for(i = 0; i < nlon; ++i)
        {
            if((-1.0 > x[j  ][im1]) || (-1.0 > x[j  ][i]) ||
               (-1.0 > x[j+1][im1]) || (-1.0 > x[j+1][i]))
		continue;
            ...
            else
            {
                if(0.0 > x[j+1][im1])
                {
                    ...
                    else
                    {
                      ...
                      //x[j+1][im1]=-0.99985, x[j+1][i]=0.999529
		      //x[j  ][im1]=-0.99947, x[j  ][i]=0.999907

                        ++numbProcessedBoundaryPoints;

                        if((_fillValue > v[0][0]) || (_fillValue > v[0][1]) ||
                           (_fillValue > v[1][0]) || (_fillValue > v[1][1]))
		            continue;
                        ...
                        if(1.0e-10 < fabs(dd))
                            ...
                            dx = 0.5;
                        y1[1] = (1.0 - dx) * y[j+1][im1] + dx * y[j+1][im1];
                        v1[1] = (1.0 - dx) * v[1][0] + dx * v[1][1];
                        f1[1] = _vscale*(v1[1] - _valmin);
...
void HDF2dViewer::_evaluate(double *var)
{
    int size;
    int n = 0;
    char vn[128];
    ...
    size = geometry->get_nlon()
         * geometry->get_nlat() 
         * geometry->get_nlev();

    cout << "\n" << __PRETTY_FUNCTION__ << ", file: " << __FILE__ << ", line: " << __LINE__ << endl;
    cout << "\t_varname: <" << _varname << ">" << endl;
    cout << "\tnlon = " << geometry->get_nlon()
         << ", nlat = " << geometry->get_nlat()
         << ", nlev = " << geometry->get_nlev() << endl;
...
void HDF2dViewer::_displayXflat(int xl)
{
    int j, k, n;
...
void HDF2dViewer::_displayYflat(int yl)
{
    int i, k, n;
