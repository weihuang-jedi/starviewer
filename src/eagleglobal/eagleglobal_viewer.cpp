...
EagleGlobal2dViewer::~EagleGlobal2dViewer()
{
    locator->turnOff();
...
void EagleGlobal2dViewer::set_geometry(EagleGlobalGeometry *gm)
{
    geometry = gm;
...
void EagleGlobal2dViewer::setup(string vn, float *var)
{
    reset();
...
void EagleGlobal2dViewer::reset()
{
    lister->reinitialize(_nlon+1, _nlat+1, geometry->get_nlev()+1);
}

void EagleGlobal2dViewer::_initialize()
{
    int i, j, m, n;
...
void EagleGlobal2dViewer::_lonlat2xyz(double lon, double lat, double radius, double fact)
{
...
void EagleGlobal2dViewer::_lonlat2xyz_texture(double lon, double lat,
...
void EagleGlobal2dViewer::_sphereDisplay()
{
    int i, j, k, k1;
    size_t mpos, npos;
  ...
  //#pragma omp parallel for
    ...
    for(j = 1; j < _nlat; ++j)
    {
        ...
        for(i = 0; i < _nlon; ++i)
        {
            ...
            _lonlat2xyz_texture(_longitude[i], _latitude[j], radius, fact);

            fact = sv * (pltvar[mpos+i] - _valmin);
            _lonlat2xyz_texture(_longitude[i], _latitude[j-1], radius, fact);
        }
        fact = sv * (pltvar[npos] - _valmin);
        _lonlat2xyz_texture(_longitude[0], _latitude[j], radius, fact);

        fact = sv * (pltvar[mpos] - _valmin);
        _lonlat2xyz_texture(_longitude[0], _latitude[j-1], radius, fact);
    glEnd();
    }
...
void EagleGlobal2dViewer::_flatDisplay()
{
    int i, j, k, k1;
    size_t mpos, npos;
    double sv = 1.0;
...
void EagleGlobal2dViewer::_evaluate(float *var)
{
    size_t varsize;
    size_t n = 0;
    float total;
    ...
    // cout << "\t geometry->get_nlon(): " << geometry->get_nlon() << endl;
    // cout << "\t geometry->get_nlat(): " << geometry->get_nlat() << endl;
...
void EagleGlobal2dViewer::_adjust_minmax(float *var)
{
    size_t varsize;
    size_t n = 0;
    float total;
...
void EagleGlobal2dViewer::reset_texture1d(ColorTable *ct)
{
    colorTable = ct;
...
void EagleGlobal2dViewer::draw_sphere_grids()
{
    int i, j, k;
    size_t npos;
...
void EagleGlobal2dViewer::draw_plane_grids()
{
    int i, j;
    size_t npos;
...
void EagleGlobal2dViewer::_sphereBump()
{
    int i, j, k, k1;
    size_t mpos, npos;
    double sv = 1.0;
  ...
  //glShadeModel(GL_SMOOTH);
    ...
    for(j = 1; j < _nlat; ++j)
    {
        ...
        {
            fact = sv * (pltvar[npos+i] - _valmin);
            _lonlat2xyz(_longitude[i], _latitude[j], radius + magnifier*fact, fact);

            fact = sv * (pltvar[mpos+i] - _valmin);
            _lonlat2xyz(_longitude[i], _latitude[j-1], radius + magnifier*fact, fact);
        }
        fact = sv * (pltvar[npos] - _valmin);
        _lonlat2xyz(_longitude[0], _latitude[j], radius + magnifier*fact, fact);

        fact = sv * (pltvar[mpos] - _valmin);
        _lonlat2xyz(_longitude[0], _latitude[j-1], radius + magnifier*fact, fact);
        glEnd();
    }
...
void EagleGlobal2dViewer::_flatBump()
{
    int i, j, k, k1;
    size_t mpos, npos;
    double sv = 1.0;
...
void EagleGlobal2dViewer::_draw_cross(double radius)
{
    int m, n;
    double x, y, z;
...
double EagleGlobal2dViewer::_k2h(int k)
{
    double height = 0.5 * ((double) (_nlev-k) / _nlev);
    return height;
}

double EagleGlobal2dViewer::_k2r(int k)
{
