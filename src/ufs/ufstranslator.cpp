//$Id: ufstranslator.cpp 4947 2014-02-04 16:09:40Z starviewer $
...
//Constructor
UFSTranslator::UFSTranslator(ColorTable *ct, NVOptions* opt, string flnm,
             ...
             : GeneralTranslator(ct, opt, flnm, isFileList, parent)
{
  ...
  //cout << "\t_filename.c_str(): <" << _filename.c_str() << ">, _hasFileList = " << _hasFileList << endl;
    ...
    ufs_controller = new UFS_Controller(colorTable, nvoptions,
...
{
    _varinfo = "Dim nx="+QString::number(ufs_controller->get_nx())
             + ", ny=" + QString::number(ufs_controller->get_ny())
             + ", nz=" + QString::number(ufs_controller->get_nz())
...
void UFSTranslator::update_sliderNspin()
{
    int intvl = 0;
    int stepsize = 1;
    if(! _first_time)
    ...
    sliderNspinX->set_max(ufs_controller->get_nx());
    sliderNspinX->setValue(ufs_controller->get_nx());
    intvl = (ufs_controller->get_nx()+9)/10;
    sliderNspinX->set_tickinterval(intvl);
    stepsize = (ufs_controller->get_nx() + 350)/360;
    sliderNspinX->set_step(stepsize);

    sliderNspinY->set_max(ufs_controller->get_ny());
    sliderNspinY->setValue(ufs_controller->get_ny());
    intvl = (ufs_controller->get_ny()+9)/10;
    sliderNspinY->set_tickinterval(intvl);
    stepsize = (ufs_controller->get_ny() + 170)/180;
    sliderNspinY->set_step(stepsize);

    sliderNspinZ->set_max(ufs_controller->get_nz());
  ...
  //cout << "\tufs_controller->get_nx() = " << ufs_controller->get_nx() << endl;
  //cout << "\tufs_controller->get_ny() = " << ufs_controller->get_ny() << endl;
  //cout << "\tufs_controller->get_nz() = " << ufs_controller->get_nz() << endl;
}
...
void UFSTranslator::selectColorMap(const QString& str)
{
  ...
  //cout << "ColorMap: <" << str.toStdString() << "> is selected." << endl;
    ...
    ufs_controller->update_colormap();
...
{
    string* varnames = ufs_controller->get_ndvNames(n);
    return varnames;
}
