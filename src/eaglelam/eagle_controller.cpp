//$Id: eagle_controller.cpp 5339 2015-02-22 04:21:52Z starviewer $
...
EAGLE_Controller::EAGLE_Controller(ColorTable *ct, NVOptions* opt,
              ...
              :     Controller(ct, opt, fn, isList)
{
  ...
  //cout << "\tfunction: <" << __PRETTY_FUNCTION__ << ">, in file: <" << __FILE__ << ">, at line: " << __LINE__ << endl;
    ...
    eagle_geometry = new EAGLE_Geometry();
...
EAGLE_Controller::~EAGLE_Controller()
{
  //cout << "\tEnter function: <" << __PRETTY_FUNCTION__ << ">, in file: <" << __FILE__ << ">, at line: " << __LINE__ << endl;
    ...
    if(NULL != trajectory)
        delete trajectory;
    if(NULL != eagle_geometry)
        delete eagle_geometry;
    if(NULL != nvfile)
        delete nvfile;
    if(NULL != eagle_glviewer)
        delete eagle_glviewer;

  //cout << "\tLeave function: <" << __PRETTY_FUNCTION__ << ">, in file: <" << __FILE__ << ">, at line: " << __LINE__ << endl;
} 

void EAGLE_Controller::setup()
{
    size_t i;
    size_t xy_size;
    size_t gridsize;
    ...
    eagle_geometry->set_nx(_nx);
    eagle_geometry->set_ny(_ny);
    eagle_geometry->set_nz(_nz);
    eagle_geometry->set_nt(_nt);

    eagle_geometry->set_lon(nvfile->get_dv("XLONG"));
    eagle_geometry->set_lat(nvfile->get_dv("XLAT"));
    eagle_geometry->set_ulon(nvfile->get_dv("XLONG_U"));
    eagle_geometry->set_ulat(nvfile->get_dv("XLAT_U"));
    eagle_geometry->set_vlon(nvfile->get_dv("XLONG_V"));
    eagle_geometry->set_vlat(nvfile->get_dv("XLAT_V"));
    ...
    eagle_geometry->set_hgt(ph);

    free(phb);

    eagle_geometry->unset_Xstaggered();
    eagle_geometry->unset_Ystaggered();
    eagle_geometry->unset_Zstaggered();

    _varname = "HGT";
    _value = nvfile->get_dv(_varname);
    _title = nvfile->get_title();
    _setup_eagle_timestring();

    _varsize = nvfile->get_varsize();
    eagle_geometry->set_nx(_varsize[2]);
    eagle_geometry->set_ny(_varsize[1]);
    eagle_geometry->set_nz(_varsize[0]);

    _nx = eagle_geometry->get_nx();
    _ny = eagle_geometry->get_ny();
    _nz = eagle_geometry->get_nz();

    eagle_glviewer = new EAGLE_Viewer(colorTable, nvoptions);
    eagle_glviewer->set_geometry(eagle_geometry);
    eagle_glviewer->setup(_varname, _value);

    _minval = eagle_glviewer->get_minval();
    _maxval = eagle_glviewer->get_maxval();

    _initialized = true;

    geometry = eagle_geometry;
}

void EAGLE_Controller::set2dvarname(string vn)
{
    _varname = vn;
  ...
  //cout << "\teagle_geometry->get_mx() = " << eagle_geometry->get_mx() << endl;
  //cout << "\teagle_geometry->get_my() = " << eagle_geometry->get_my() << endl;
  //cout << "\teagle_geometry->get_mz() = " << eagle_geometry->get_mz() << endl;
  //cout << "\teagle_geometry->get_nx() = " << eagle_geometry->get_nx() << endl;
  //cout << "\teagle_geometry->get_ny() = " << eagle_geometry->get_ny() << endl;
  //cout << "\teagle_geometry->get_nz() = " << eagle_geometry->get_nz() << endl;

    _initialized = true;

    if(eagle_geometry->get_nx() == _mx)
        eagle_geometry->set_Xstaggered();
    if(eagle_geometry->get_ny() == _my)
        eagle_geometry->set_Ystaggered();

    eagle_glviewer->setup(vn, _value);
    eagle_glviewer->setup(_varname, _value);
...
void EAGLE_Controller::set3dvarname(string vn)
{
    _varname = vn;
  ...
  //cout << "\teagle_geometry->get_nz() = " << eagle_geometry->get_nz() << endl;
    ...
    if(eagle_geometry->get_ny() == _my)
        eagle_geometry->set_Ystaggered();
    if(eagle_geometry->get_nz() == _mz)
        eagle_geometry->set_Zstaggered();
    ...
    {
        marchingCube.reset();
      //marchingCube.setup(eagle_geometry->get_nx(), eagle_geometry->get_ny(), eagle_geometry->get_nz(),
      //                  &_value[0], _minval, _maxval);
    }
}

void EAGLE_Controller::_setup4eagle()
{
    int n;
  ...
  //cout << "\tnatts = " << natts << endl;
    ...
    for(n = 0; n < natts; ++n)
    {
      ...
      //    eagle_geometry->set_model(EAGLE);

        attvalue = nvfile->get_att_value("MMINLU");
        if(0 == attvalue.compare("unknown"))
             eagle_geometry->set_mminlu(attvalue.c_str());
        ...
        {
            iv = string2number<int>(attvalue);
            eagle_geometry->set_parent_id(iv);
        }
        ...
        {
            iv = string2number<int>(attvalue);
            eagle_geometry->set_parent_grid_ratio(iv);
        }
        ...
        {
            iv = string2number<int>(attvalue);
            eagle_geometry->set_i_parent_start(iv);
        }
        ...
        {
            iv = string2number<int>(attvalue);
            eagle_geometry->set_j_parent_start(iv);
        }
        ...
        {
            iv = string2number<int>(attvalue);
            eagle_geometry->set_map_proj(iv);
        }
        ...
        {
            iv = string2number<int>(attvalue);
            eagle_geometry->set_num_land_cat(iv);
        }
        ...
        {
            iv = string2number<int>(attvalue);
            eagle_geometry->set_issoilwater(iv);
        }
        ...
        {
            fv = string2number<float>(attvalue);
            eagle_geometry->set_truelat1(fv);
        }
        ...
        {
            fv = string2number<float>(attvalue);
            eagle_geometry->set_truelat2(fv);
        }
        ...
        {
            fv = string2number<float>(attvalue);
            eagle_geometry->set_moad_cen_lat(fv);
        }
        ...
        {
            fv = string2number<float>(attvalue);
            eagle_geometry->set_stand_lon(fv);
        }
        ...
        {
            fv = string2number<float>(attvalue);
            eagle_geometry->set_pole_lat(fv);
        }
        ...
        {
            fv = string2number<float>(attvalue);
            eagle_geometry->set_pole_lon(fv);
        }
        ...
        {
            fv = string2number<float>(attvalue);
            eagle_geometry->set_gmt(fv);
        }
    }
}

void EAGLE_Controller::set_fileNtime(int nf, int nt)
{
    size_t gridsize = 1;
  ...
  //cout << "\tnvoptions->get_zsec() = " << nvoptions->get_zsec() << endl;
    ...
    if(_preFile != _curFile)
    {
        ...
        eagle_geometry->set_nt(_ntimes[_curFile]);
        ...
        eagle_glviewer->setup(_varname, &_value[gridsize]);
...
void EAGLE_Controller::update_colormap()
{
    eagle_glviewer->update_colormap();

    if(nvoptions->get_cb(NV_ISOSURFACEON))
        marchingCube.reset();
}

void EAGLE_Controller::draw_isosurface()
{
...
void EAGLE_Controller::draw_vector()
{
  //cout << "\nIn functions: <" << __PRETTY_FUNCTION__ << ">, line: "
  //     << __LINE__ << ", file: <" << __FILE__ << ">" << endl;
...
void EAGLE_Controller::draw_trajectory()
{
  //cout << "\nIn functions: <" << __PRETTY_FUNCTION__ << ">, line: "
  //     << __LINE__ << ", file: <" << __FILE__ << ">" << endl;
    ...
    {  
        if(NULL == trajectory)
        {
            ...
            trajectory->set_dtNdx(eagle_geometry->get_dt(), eagle_geometry->get_dx());
...
void EAGLE_Controller::draw_lic()
{
...
void EAGLE_Controller::_setup_eagle_timestring()
{
    int n = 0;
    int ns = 0;
    int dsl = nvfile->get_dim_size("DateStrLen");
    int wtsl = eagle_timestring.size();
    ...
    if(wtsl < _nt)
    {
        ...
        for(n = wtsl; n < _nt; ++n)
        {
          ...
          //nstr = "Time: " + number2string<int>(n) + ", " + item;
            nstr = string(", ") + item;
            eagle_timestring.push_back(nstr);

          //cout << "\nIn functions: <" << __PRETTY_FUNCTION__ << ">, line: "
          //     << __LINE__ << ", file: <" << __FILE__ << ">" << endl;
          //cout << "\teagle_timestring[" << n << "] = " << eagle_timestring[n] << endl;
        }
    }
    else
    {
        for(n = 0; n < wtsl; ++n)
        {
          ...
          //eagle_timestring[n] = "Time: " + number2string<int>(n) + ", " + item;
            eagle_timestring[n] = string(", ") + item;
        }
    }
...
string* EAGLE_Controller::get_timestring()
{
    int n = 0;
    ...
    for(n = 0; n < _nt; ++n)
    {
        timestring[n] += eagle_timestring[n];
...
void EAGLE_Controller::unset_vector()
{
    drawWindVector = false;
    if(NULL != ua)
        free(ua);
...
void EAGLE_Controller::setup_vector()
{
    size_t i;
    size_t xy_size;
    size_t gridsize;
...
void EAGLE_Controller::_get_vector(int nt)
{
    int i, j, k;
    size_t n, nu, nv, nw;
...
void EAGLE_Controller::set_locator(Locator* l)
{
     locator = l;

     eagle_glviewer->set_locator(l);
}

int EAGLE_Controller::get_ndv(int n)
{
    return nvfile->get_ndv(n);
}

string* EAGLE_Controller::get_ndvNames(int n)
{
